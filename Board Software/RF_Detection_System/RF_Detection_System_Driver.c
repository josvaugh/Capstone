...
//#define SERVER_IP "" //Guy's Spare PC
#define SERVER_IP "149.162.183.120" //Joey's Public Laptop
//Original IP 68.44.201.181
...
int main(void)
{
	...
	/*
		This might be the function that detects if it's online?
		"converts the Internet host address cp from the
		IPv4 numbers-and-dots notation into binary form,
		(in network byte order) and stores it in the structure that inp points to."
	*/
	...
	while (1)
	{
		//Offline Mode First; checks to see if there is an internet connection
		if (sendto(sock, packet, PACKET_SIZE, 0, (struct sockaddr *) &si_other, slen) == -1)
		{
			...
			if (sig_strength > SIGNAL_THRESHOLD && speed >= 1)
			{
				...
				printf("              \n");
				system("tput cup 3 0");
				printf("LAST CAPTURE IN OFFLINE MODE\n");
				printf("=====================================\n");
			...
			//once max packets are stored, reboot system to try to solve the issue and offload
				...
				if (msg_len == -1)
				{
					...
					/*
						Both loops will contain this detection.
					*/
					...
					{
						/*
						FUTURE NOTE: If this does happen, there needs to be a backup file instead
						so it can continuously keep scanning for data.
						Example, it should change the current data file name to date:packet_storage,
						and create a new file simply called packet_storage.
						*/
						//REBOOT
						fclose(fptr);
						printf("Storage Full!\n");
		...
		else
		{
			...
			if (file_size != 0)
			{
				...
				while (ftell(fptr) < file_size)
				{
					...
					{
						/*
							Still a viable if; but it should continue to the offline stage
					...
					/*
						Instead, if timeout is met, continue to offline mode
					*/
					len = sizeof si_other;
					msg_len = recvfrom(sock, (char *)buf, BUF_SIZE, MSG_WAITALL, (struct sockaddr *) &si_other, &len);
					if (msg_len == -1)
				...
				/*
					Clear packet only if in online mode.
				*/
			...
			while (1)
			{
				...
				if (sig_strength > SIGNAL_THRESHOLD)
				{
					...
					{
						/*
							Instead, this will notify the interface saying there is a disconnect,
							then search for an offline solution instead.
						*/
						perror("sendto() failed! Is hotspot on?");
						//failed to connect to hotspot, reboot and try again
						system("sudo reboot");
					...
					/*
						This if statement will be on the online while loop, while the offline
						will exclude it entirely.
					*/
					//Recieve confirmation message
					//if timeout is met, store the packet until max packets are stored
					//once max packets are stored, reboot system to try to solve the issue and offload
